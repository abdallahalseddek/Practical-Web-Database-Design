...
| The Query                                                                                                                                                                                                                   | Execution time before Optimization | Optimization Technique                                                              | Rewrite Query                                                                                                                                                                                                                                                                                                                           | Execution time After Optimization |
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------:|-------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------:|
| `select count(*) from product p join category cat on cat.category_id=p.category_id where p.category_id=10;`                                                                                                                 |                1 ms                | create a composite key on `product` and `orders` tables                             | `create index order_prod on order_details(product_id,order_id);` `create index idx_order_cus on orders(order_id,total_amount);`                                                                                                                                                                                                         |               .9 ms               |
| `select c.first_name, c.last_name, o.order_id,o.total_amount from customer c join orders o on c.customer_id = o.customer_id where  o.total_amount = (SELECT MAX(total_amount) from orders);`                                |               1.9 ms               | using `ORDER BY` and `LIMIT` instead of using `MAX` and index `total_amount` column | `SELECT c.first_name, c.last_name, o.order_id, o.total_amount FROM customer c JOIN orders o ON c.customer_id = o.customer_id ORDER BY o.total_amount DESC LIMIT 1;` `SELECT c.first_name, c.last_name, o.order_id, o.total_amount FROM customer c JOIN orders o ON c.customer_id = o.customer_id ORDER BY o.total_amount DESC LIMIT 1;` |               .7 ms               |
| `select p.product_id,p.name,p.stock_quantity from product p where stock_quantity<10;`                                                                                                                                       |               1.9 ms               | using composite index of `product_id, name, stock_quantity`                         | `create index idx_qu_prod on product(product_id,name,stock_quantity);`                                                                                                                                                                                                                                                                  |              1.7 ms               |
